...
Iterate over file names and read in data for each subject using `purrr::map` and save the result as a new variable.
...
Tidy the results to include control arm and subject ID. Create a long format dataset so that values can be plotted over time in the following plot.
...
Individuals in the experimental arm were more likely to have increases in their value over time. Individuals in the control arm stayed roughly the same.

# Problem 3

In this problem, I will simulate data to explore power in a one-sample t-test. Before beginning, I set the seed so results are reproducible. 
